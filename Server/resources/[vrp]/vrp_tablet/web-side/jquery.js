...
$(document).ready(function(){
	...
	window.addEventListener("message",function(event){
		switch (event.data.action){
			...
			case "messageMedia":
				updateMedia(item.media,item.data)
			break;
		};
	});
	...
	document.onkeyup = function(data){
		if (data.which == 27){
			$.post("http://vrp_tablet/closeSystem");
		};
	};
});
/* ---------------------------------------------------------------------------------------------------------------- */
...
const commandsPage = () => {
	...
	$('#content').html(`
		...
		<div id='pageDiv'>
			...
			<b>trunk</b> - Abrir porta-malas do veículo<br>
		...
		<div id='pageDiv'>
			...
			<b>garmas</b> - Guarda o armamento na mochila<br>
			<b>me</b> - Efetuar uma animação não existente<br>
			<b>sex M/F</b> - Muda o sexo do personagem<br>
		...
		<div id='pageDiv'>
			<b>F1</b> - Abrir a mochila<br>
			<b>F2</b> - Abrir o tablet<br>
...
const midiaPage = (data) => {
	...
	$("#socialRight").html("")
	$("#socialRight").html("<div id='socialTitle'>"+data+"</div><div id='socialContent'></div>")
	$.post("http://vrp_tablet/requestMedia",JSON.stringify({ media: data }),(data) => {
		$.each(JSON.parse(data),(k,v) => {
			$('#socialContent').prepend(`<div id="socialLine" ${v.back === true ? "style='background: rgba(255,250,189,0.3); color: #5b5b27;'":""}>${v.text}</div>`);
		})
	});
}
/* ---------------------------------------------------------------------------------------------------------------- */
const socialPage = () => {
	...
	$('#content').html(`
		<div id='titleContent'>MÍDIAS SOCIAIS</div>
		<div id='socialLeft'>
			<li onclick='midiaPage("Twitter")' style='margin-right: 1px'>T</li>
			<div id='socialDivision'></div>
			<li onclick='midiaPage("Policia")'>P</li>
			<div id='socialDivision'></div>
			<li onclick='midiaPage("Paramedico")'>P</li>
			<div id='socialDivision'></div>
			<li onclick='midiaPage("Anonimo")' style='margin-left: 1px'>A</li>

			<textarea maxlength='280' class='mediaArea' spellcheck='false' value='' placeholder='Escreva uma mensagem...'></textarea>
			<div id='socialSubmit'>ENVIAR MENSAGEM</div>
		</div>
...
$(document).on("click","#socialSubmit",function(){
	...
	if (messageMedia != "") {
		$.post("http://vrp_tablet/messageMedia",JSON.stringify({ message: messageMedia, page: mediaPage }));
		$('.mediaArea').val("")
	}
});
/* ----------UPDATEMEDIA---------- */
const updateMedia = (media,data) => {
	if (mediaPage == media){
		$('#socialContent').prepend(`<div id="socialLine" ${data.back === true ? "style='background: rgba(255,250,130,0.3); color: #5b5b27;'":""}>${data.text}</div>`);
	}
}