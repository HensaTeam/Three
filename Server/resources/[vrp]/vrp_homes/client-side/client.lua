    ...
    while true do
        ...
        for k,v in pairs(homesList) do
            ...
            if distance <= 1.5 then
                timeDistance = 5
                DrawText3Ds(v[5],v[6],v[7],"~g~/~w~ENTER    |    ~g~/~w~TRANCAR")
            end
        end
					...
					if not theftPlayers[k] then
						...
						if distance <= 2.0 then
							...
							if distance <= 0.9 and IsControlJustPressed(1,38) then
								...
								if k == "LOCKER" then
									local homeLocker = exports["vrp_safelocker"]:createSafe({ math.random(0,99),math.random(0,99) })
									if homeLocker then
										vSERVER.paymentTheft(k)
									end
									theftPlayers[k] = true
								else
									local taskBar = exports["vrp_taskbar"]:taskThree()
									if taskBar then
										vSERVER.paymentTheft(k)
										theftPlayers[k] = true
									end
				...
				if distance <= 1.5 then
					...
					if IsControlJustPressed(1,38) then
						if v[4] == "exit" then
							...
							TriggerEvent("vrp_homes:Hours",false)
							FreezeEntityPosition(ped,true)
