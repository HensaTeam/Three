...
function cRP.deleteChar()
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		vRP.Query("vRP/remove_characters",{ id = parseInt(user_id) })
		Citizen.Wait(1000)
			...
			if distanceService then
				if service == "Police" then
					if vRP.hasPermission(user_id,"Police") then
						...
						vRP.Query("vRP/upd_group",{ user_id = user_id, permiss = "Police", newpermiss = "waitPolice" })
					elseif vRP.hasPermission(user_id,"waitPolice") then
						...
						vRP.Query("vRP/upd_group",{ user_id = user_id, permiss = "waitPolice", newpermiss = "Police" })
					end
				end
				...
				if service == "Paramedic" then
					if vRP.hasPermission(user_id,"Paramedic") then
						...
						TriggerEvent("vrp_blipsystem:serviceExit",source)
						TriggerClientEvent("Notify",source,"importante","Você saiu de serviço.",5000)
						vRP.Query("vRP/upd_group",{ user_id = user_id, permiss = "Paramedic", newpermiss = "waitParamedic" })
					elseif vRP.hasPermission(user_id,"waitParamedic") then
						...
						TriggerClientEvent("Notify",source,"importante","Você entrou em serviço.",5000)
						vRP.Query("vRP/upd_group",{ user_id = user_id, permiss = "waitParamedic", newpermiss = "Paramedic" })
					end
				end
			end
		end
		...
		if parseInt(args[2]) > 0 then
			if args[1] == "Police" then
				if vRP.hasPermission(user_id,"PolMaster") then
					vRP.Query("vRP/cle_group",{ user_id = parseInt(args[2]) })
					vRP.Query("vRP/add_group",{ user_id = parseInt(args[2]), permiss = tostring("waitPolice") })
			...
			if args[1] == "Paramedic" then
				if vRP.hasPermission(user_id,"ParMaster") then
					vRP.Query("vRP/cle_group",{ user_id = parseInt(args[2]) })
					vRP.Query("vRP/add_group",{ user_id = parseInt(args[2]), permiss = tostring("waitParamedic") })
					TriggerClientEvent("Notify",source,"sucesso","Passaporte <b>"..vRP.format(parseInt(args[2])).."</b> adicionado com sucesso.",5000)
				end
			end
		end
	...
	if user_id then
		if parseInt(args[1]) > 0 then
			if vRP.hasPermission(user_id,"PolMaster") or vRP.hasPermission(user_id,"MecMaster") or vRP.hasPermission(user_id,"ParMaster") then
				vRP.Query("vRP/cle_group",{ user_id = parseInt(args[1]) })
