...
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local inLocate = { 15.78,-1604.75,29.38 }
		...
		if distance <= 5 then
			...
			if distance <= 1 and IsControlJustPressed(1,38) then
				if inService then
					...
					inService = true
					TriggerEvent("Notify","sucesso","Serviço iniciado.",3000)
				end
			end
		end
	...
	while true do
		if inService and inTimers > 0 then
			...
			if inTimers <= 0 then
				...
				TriggerEvent("NotifyPush",{ code = 20, title = "Quero comprar um produto", x = pedLocate[rand][1], y = pedLocate[rand][2], z = pedLocate[rand][3], name = callName[math.random(#callName)].." "..callName2[math.random(#callName2)], rgba = {69,115,41} })
			end
		end
	...
	while true do
		if timeSelling > 0 then
			...
			if timeSelling <= 0 then
				...
				if (lastItem == "joint" or lastItem == "cocaine") and math.random(100) >= 95 then
